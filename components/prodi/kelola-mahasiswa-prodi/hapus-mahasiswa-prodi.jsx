...
export const HapusMahasiswaProdi = ({ data }) => {
  ...
  const handleDelete = async () => {
    try {
      setIsLoading(true);
      await axios.delete(
        `${process.env.API_BASE_URL}/mahasiswa/delete/prodi/${data.id_mahasiswa}`,
        {
          headers: {
      ...
      window.location.href = "/kelola-mahasiswa-prodi";
    } catch (error) {
      console.log(error);
    } finally {
      setIsLoading(false);
  ...
  return (
    <div>
      <Tooltip content="Hapus Mahasiswa" color="error" onClick={handler}>
        <IconButton>
